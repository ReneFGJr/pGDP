...
class socials extends CI_Model {
	...
	function button_login_modal() {
		...
		$data['email_ok'] = '';
		$data['error'] = '';
		$sx = $this -> load -> view('login/login.php', $data, true);
		return ($sx);
	}
	...
	function login() {
		$this -> load -> view('login/login_signin', null);
	}
	...
	function email_cab() {
		...
		$sx .= '<font color="blue" style="font-size: 24px;">PGDP</font>' . cr();
		$sx .= '<br>';
		$sx .= '<font color="blue" style="font-size: 12px;"><i>Plano de Gestão</i></font>' . cr();
		$sx .= '<hr>';
		return ($sx);
	}
	...
	function signup() {
		...
		if ((strlen($name) > 0) and (strlen($email))) {
			...
			if ($rs == 1) {
				...
				$this -> load -> view('login/login_signup_sucess', $dt);
				return ("");
			}
		}
